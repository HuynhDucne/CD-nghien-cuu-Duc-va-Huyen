...
/**
 * TUWFI (Top k Uncertain Weighted Frequent Itemsets) Algorithm - Business Logic Layer:
...
public class TuwfiBll {
    ...
    public static void setTopKCupOneItem(List<Cup> topKCupOneItem) {
        TuwfiBll.topKCupOneItem = topKCupOneItem;
    }

    /**
     * Thực thi toàn bộ thuật toán TUWFI
     *
     ...
     */
    public static List<Cup> executeTuwfi(String probFile, String weightFile, int k) {

        System.out.println("TUWFI Algorithm is running...");
        ...
        TuwfiDal.loadWeight(weightFile);
        ...
        setTopKCupOneItem(TuwfiDal.truncateCupList(TuwfiDal.generateCupListForItems(probFile), k));

        // Copy các phần tử từ topKCupOneItem vào result
        List<Cup> result = new ArrayList<>(topKCupOneItem);

        // Gọi hàm TuwfiSearch để tìm kiếm CUP itemset mới phù hợp để đưa vào result
        TuwfiSearch(result, topKCupOneItem);
    ...
    /**
     * Tính toán TEP-List cho CUP sau khi kết hợp. Sử dụng vòng lặp để so sánh
     *
    ...
    private static List<Tep> getTepListOfCombinedCup(Cup cup1, Cup cup2) {
        ...
        List<Tep> tepList2 = cup2.getTepList();
        ...
        for (Tep t1 : tepList1) {
            for (Tep t2 : tepList2) {
                if (t1.getTid() == t2.getTid()) {
                    double prob1 = t1.getProb();
                    double prob2 = t2.getProb();
                    double prob = (prob1 * prob2);
                    Tep newTep = new Tep(t1.getTid(), prob);
    ...
    /**
     * Thực hiện thuật toán TUWFI (Top-k Uncertain Weighted Frequent Itemsets) Search trên CUP-List để tìm Top-k UWFIs
     * TuwfiSearch thực hiện chiến lược chia để trị. Với mẫu i (itemset có kích thước i),
     ...
     * @param candidates Top-k Cup item đơn ban đầu được sắp xếp giảm dần theo CumWeightSup
     */
    public static void TuwfiSearch(List<Cup> result, List<Cup> candidates) {
        /*
        ...
        for (int i = 0; i < sizeCandidates - 1; i++) {
            Cup currentCup = candidates.get(i);

            List<Cup> nextCandidates = new ArrayList<>();

            for (int j = i + 1; j < sizeCandidates; j++) {
                Cup nextCup = candidates.get(j);
                ...
                Cup realNextCup = TuwfiDal.getCupByItemName(topKCupOneItem, lastItemNextCup);
                ...
                for (String itemName : mergedItemsNameList) {
                    if (TuwfiDal.getWeightMap().containsKey(itemName)) {
                        count++;
                        sumWeight += TuwfiDal.getWeightMap().get(itemName);
                    }
                }
                 ...
                 * (= weight * cumWeightSup của CUP thứ nhất * calculateMaxProb của CUP thứ 2)
            ...
            // Nếu nextCandidates không trống thì tiếp tục gọi TuwfiSearch
            if (!nextCandidates.isEmpty()) {
                TuwfiSearch(result, nextCandidates);
            }
        }
    }
    ...
    public static void printStats() {
        System.out.println("============= TOP-K UNCERTAIN WEIGHTED FREQUENT ITEMSETS - STATS =============");
        System.out.println(" Top K = " + topK);
        System.out.println(" Items count from dataset: " + TuwfiDal.getItemsNameList().size());
        System.out.println(" Transactions count from dataset : " + TuwfiDal.getTransactionCount());
     ...
     */
    public static void printTuwfi(List<Cup> result) {
        System.out.println("============= PRINT TOP-K UNCERTAIN WEIGHTED FREQUENT ITEMSETS =============");
        int no = 0;
        for (Cup cup : result) {
            no++;
    ...
    public static void writeResultsToFile(List<Cup> result, String resultFile) {
        ...
        File dir = new File(System.getProperty("user.dir") + "/src/algorithms/tuwfi/result");
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(new File(dir, resultFile)))) {
            writer.write("============= PRINT TOP-K UNCERTAIN WEIGHTED FREQUENT ITEMSETS =============\n");
            int no = 1;
            ...
            writer.write("============= TOP-K UNCERTAIN WEIGHTED FREQUENT ITEMSETS - STATS =============\n");
            writer.write(" Top K = " + topK + "\n");
            writer.write(" Items count from dataset: " + TuwfiDal.getItemsNameList().size() + "\n");
            writer.write(" Transactions count from dataset : " + TuwfiDal.getTransactionCount() + "\n");
